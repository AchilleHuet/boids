...
class Boid():
    ...
    def update(self, neighbors):
        # update position
        self.acc = self.cohesion(neighbors) * 0.45 + self.alignment(neighbors) * 0.5 + self.separation(neighbors) * 0.5 + self.avoidance() * 10
    ...
    def draw(self, screen):
        """Draws the boid on the given screen"""
    ...
    def cohesion(self, neighbors):
        """Finds the average direction to nearby boids"""
    ...
    def avoidance(self):
        ...
        dist_right = (SCREEN - self.pos)[0][0]
        dist_top = self.pos[1][0]
        dist_bot = (SCREEN - self.pos)[1][0]
